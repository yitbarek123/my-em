(define-module (opencog ghost procedures)
  ...
  #:use-module (virtual-assistant virtual-assistant)
  #:use-module (opencog)
...
"
  (if (> timer-last-stimulated 0)
    (cog-stimulate timer-predicate
      (* 10 (- (time-second (current-time)) timer-last-stimulated))))

  (set! timer-last-stimulated (time-second (current-time)))
)
...
"
  ...
  (va-push-vauttr (format "~a" (string-join (map cog-name str-nodes))))
  fini
)
